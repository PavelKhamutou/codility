...
object PrefixSet extends App {
  val ps = new PrefixSet
  val a = Array(2, 2, 1, 0, 1)
  println(ps.solution(Array(1, 2)))
}
...
class PrefixSet {
  def solution(A: Array[Int]): Int = {
    ...
    for(i <- 0 until A.length) {
      val B = A.slice(0, i)
      if(isContain(B, A)) return prefIndex
      prefIndex = i
    }
    -1
  }

  def isContain(B: Array[Int], A: Array[Int]): Boolean = {
    for(i <- 0 until A.length){
